<template>
  <div :class="'notify-' + options.position" :style="{ width: width}" class="notify">
    <transition-group name="notify" tag="div" @enter="slideDown" @leave="slideUp">
      <div @click="removeItem(key)" v-for="(item, key) in items" :key="key" :class="item.options.itemClass" class="alert d-flex align-items-center" role="alert">
        <div v-if="item.options.iconClass" class="flex-00-auto">
            <i :class="item.options.iconClass"></i>
        </div>
        <div v-if="item.options.mode === 'html'" v-html="item.text" class="flex-fill ml-3"></div>
        <div class="flex-fill ml-3">
          <p v-if="item.options.mode" class="mb-0" v-text="item.text"></p>
        </div>
      </div>
    </transition-group>
  </div>
...
export default {
  data () {
    return {
      types: {
        info: { itemClass: 'alert-info', iconClass: 'fa fa-fw fa-info-circle' },
        error: { itemClass: 'alert-danger', iconClass: 'fa fa-fw fa-times-circle' },
        warning: { itemClass: 'alert-warning', iconClass: 'fa fa-fw fa-exclamation-circle' },
        success: { itemClass: 'alert-success', iconClass: 'fa fa-fw fa-check' }
      },
      options: {
        itemClass: '',
        duration: 300,
  ...
  methods: {
    ...
    slideDown (el) {
      Velocity(el, this.options.enter, {
        duration: this.options.duration
      })
    },
    slideUp (el, done) {
      Velocity(el, this.options.leave, {
        duration: this.options.duration, complete: done
      })
    },
    removeItem (index) {
